...
import { DogRaisinToxicityCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.dogRaisinToxicity?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.dogRaisinToxicity.seo' });
  const path = '/calculators/biology/dog-raisin-toxicity';
...
export default async function DogRaisinToxicityPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.dogRaisinToxicity' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });
  const tCalcs = await getTranslations({ locale, namespace: 'calculators' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.dogRaisinToxicity.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.dogRaisinToxicity.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.dogRaisinToxicity.seo.related' });
      ...
      <div className="calculator-header">
        <div className="container">
          ...
          <div className="header-content">
            <div className="title-badge">{tCalcs('biologyCalculators.badge')}</div>
      ...
      <div className="related-calculators-section">
        <div className="related-content-card">
          ...
          <div className="related-grid">
            <Link href="/calculators/real-life/age-in-days" className="related-card">
              <h3 className="related-title">{tRelated('ageInDays')}</h3>
              <p className="related-desc">{tRelated('ageInDaysDesc')}</p>
            </Link>
