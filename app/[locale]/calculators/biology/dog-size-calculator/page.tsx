...
import { DogSizeCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.dogSize?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.dogSize.seo' });
  const path = '/calculators/biology/dog-size-calculator';
...
export default async function DogSizePage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.dogSize' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });
  const tCalcs = await getTranslations({ locale, namespace: 'calculators' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.dogSize.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.dogSize.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.dogSize.seo.related' });
...
export default async function DogSizeCalculatorPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.dogSize' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });
  const tCalcs = await getTranslations({ locale, namespace: 'calculators' });
