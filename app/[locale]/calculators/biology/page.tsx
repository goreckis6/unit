...
import { biologyCalculators } from '@/lib/calculators/biology';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.biologyCalculators?.faq?.items as FaqItem[]) || [];
  } catch {
...
export default async function BiologyCalculatorsPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.biologyCalculators.faq' });
  const tSeo = await getTranslations({ locale, namespace: 'calculators.biologyCalculators.seoContent' });
  
  const faqItems = await getFaqItems(locale);
  const calculators = await getCalculatorsForCategory('biology', locale, biologyCalculators);
        ...
        <div className="container">
          <div className="calculator-header">
            ...
            <div className="header-content">
              <div className="title-badge">{t('biologyCalculators.badge') || 'Biology'}</div>
              <h1 className="page-title">{t('biologyCalculators.title')}</h1>
            </div>
          </div>
