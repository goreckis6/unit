...
import { DaysToWeeksCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.daysToWeeks?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.daysToWeeks.seo' });
  const path = '/calculators/conversion/days-to-weeks';
...
export default async function DaysToWeeksPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.daysToWeeks' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.daysToWeeks.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.daysToWeeks.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.daysToWeeks.seo.related' });
