  ...
  return (
    <>
      <CalculatorSearch 
        ...
        placeholder={t('conversionCalculators.searchPlaceholder') || 'Search calculators...'}
        resultsText={t('conversionCalculators.resultsFound') || 'results found'}
        noResultsText={t('conversionCalculators.noResults') || 'No results found'}
        noResultsHint={t('conversionCalculators.tryDifferentSearch') || 'Try a different search term'}
      />

      <div className="calculators-list">
        {filteredCalculators.map((calc) => (
          <Link key={calc.id} href={calc.path} className="calculator-list-item">
        ...
        {filteredCalculators.length === 0 && (
          <div className="no-results">
            ...
            <p className="no-results-text">{t('conversionCalculators.noResults') || 'No results found'}</p>
            <p className="no-results-hint">{t('conversionCalculators.tryDifferentSearch') || 'Try a different search term'}</p>
          </div>
        )}
      </div>
    </>
