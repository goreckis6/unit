...
import { LowercaseToUppercaseCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.lowercaseToUppercase?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.lowercaseToUppercase.seo' });
  const path = '/calculators/conversion/lowercase-to-uppercase';
...
export default async function LowercaseToUppercasePage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.lowercaseToUppercase' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.lowercaseToUppercase.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.lowercaseToUppercase.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.lowercaseToUppercase.seo.related' });
      ...
      <div className="calculator-container">
        <div className="container">
          <div className="calculator-card">
            <LowercaseToUppercaseCalculator />
          </div>
        </div>
      </div>
        ...
        <div className="container">
          <div className="faq-content-card">
            ...
            <div className="related-grid">
              <Link href="/calculators/conversion/uppercase-to-lowercase" className="related-card">
                <h3 className="related-title">{tRelated('uppercaseToLowercase')}</h3>
                <p className="related-desc">{tRelated('uppercaseToLowercaseDesc')}</p>
              </Link>
