...
import { KwhToKwCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.kwhToKw?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.kwhToKw.seo' });
  const baseUrl = 'https://unitconverterhub.com';
  const path = locale === 'en' ? '/calculators/electric/kwh-to-kw' : `/${locale}/calculators/electric/kwh-to-kw`;
  const canonicalUrl = `${baseUrl}${path}`;
  const hreflangUrls = generateHreflangUrls('/calculators/electric/kwh-to-kw');
...
export default async function KwhToKwPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.kwhToKw' });
  ...
  const tSeo = await getTranslations({ locale, namespace: 'calculators.kwhToKw.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.kwhToKw.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.kwhToKw.seo.related' });
