...
import { MahToWhCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.mahToWh?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.mahToWh.seo' });
  const baseUrl = 'https://unitconverterhub.com';
  const path = locale === 'en' ? '/calculators/electric/mah-to-wh' : `/${locale}/calculators/electric/mah-to-wh`;
  const canonicalUrl = `${baseUrl}${path}`;
  const hreflangUrls = generateHreflangUrls('/calculators/electric/mah-to-wh');
...
export default async function MahToWhPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.mahToWh' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });
  
  const faqItems = await getFaqItems(locale);
  
  const tSeo = await getTranslations({ locale, namespace: 'calculators.mahToWh.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.mahToWh.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.mahToWh.seo.related' });
