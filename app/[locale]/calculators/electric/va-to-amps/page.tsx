...
import { VaToAmpsCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.vaToAmps?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.vaToAmps.seo' });
  const baseUrl = 'https://unitconverterhub.com';
  const path = locale === 'en' ? '/calculators/electric/va-to-amps' : `/${locale}/calculators/electric/va-to-amps`;
  const canonicalUrl = `${baseUrl}${path}`;
  const hreflangUrls = generateHreflangUrls('/calculators/electric/va-to-amps');
...
export default async function VaToAmpsPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.vaToAmps' });
  ...
  const tSeo = await getTranslations({ locale, namespace: 'calculators.vaToAmps.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.vaToAmps.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.vaToAmps.seo.related' });
