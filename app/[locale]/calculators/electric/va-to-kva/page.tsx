...
import { VaToKvaCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.vaToKva?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.vaToKva.seo' });
  const baseUrl = 'https://unitconverterhub.com';
  const path = locale === 'en' ? '/calculators/electric/va-to-kva' : `/${locale}/calculators/electric/va-to-kva`;
  const canonicalUrl = `${baseUrl}${path}`;
  const hreflangUrls = generateHreflangUrls('/calculators/electric/va-to-kva');
...
export default async function VaToKvaPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.vaToKva' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.vaToKva.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.vaToKva.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.vaToKva.seo.related' });
      ...
      <div className="related-calculators-section">
        <div className="related-content-card">
          ...
          <div className="related-grid">
            ...
            <Link href="/calculators/electric/va-to-watts" className="related-card">
              <h3 className="related-title">{tRelated('vaToWatts')}</h3>
              <p className="related-desc">{tRelated('vaToWattsDesc')}</p>
            </Link>
            <Link href="/calculators/electric/kva-to-va" className="related-card">
              <h3 className="related-title">{tRelated('kvaToVa')}</h3>
              <p className="related-desc">{tRelated('kvaToVaDesc')}</p>
            </Link>
          </div>
        </div>
      </div>
