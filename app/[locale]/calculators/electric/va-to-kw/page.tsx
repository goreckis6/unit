...
import { VaToKwCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.vaToKw?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.vaToKw.seo' });
  const baseUrl = 'https://unitconverterhub.com';
  const path = locale === 'en' ? '/calculators/electric/va-to-kw' : `/${locale}/calculators/electric/va-to-kw`;
  const canonicalUrl = `${baseUrl}${path}`;
  const hreflangUrls = generateHreflangUrls('/calculators/electric/va-to-kw');
...
export default async function VaToKwPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.vaToKw' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.vaToKw.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.vaToKw.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.vaToKw.seo.related' });
      ...
      <div className="related-calculators-section">
        <div className="related-content-card">
          ...
          <div className="related-grid">
            ...
            <Link href="/calculators/electric/kva-to-kw" className="related-card">
              <h3 className="related-title">{tRelated('kvaToKw')}</h3>
              <p className="related-desc">{tRelated('kvaToKwDesc')}</p>
            </Link>
