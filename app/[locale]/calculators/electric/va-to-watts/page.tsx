...
import { VaToWattsCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.vaToWatts?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.vaToWatts.seo' });
  const baseUrl = 'https://unitconverterhub.com';
  const path = locale === 'en' ? '/calculators/electric/va-to-watts' : `/${locale}/calculators/electric/va-to-watts`;
  const canonicalUrl = `${baseUrl}${path}`;
  const hreflangUrls = generateHreflangUrls('/calculators/electric/va-to-watts');
...
export default async function VaToWattsPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.vaToWatts' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.vaToWatts.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.vaToWatts.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.vaToWatts.seo.related' });
