...
import { VoltsToKwCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.voltsToKw?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.voltsToKw.seo' });
  const baseUrl = 'https://unitconverterhub.com';
  const path = locale === 'en' ? '/calculators/electric/volts-to-kw' : `/${locale}/calculators/electric/volts-to-kw`;
  const canonicalUrl = `${baseUrl}${path}`;
  const hreflangUrls = generateHreflangUrls('/calculators/electric/volts-to-kw');
  return {
    title: t('title'),
...
export default async function VoltsToKwPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.voltsToKw' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });
  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.voltsToKw.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.voltsToKw.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.voltsToKw.seo.related' });
      ...
      <div className="related-calculators-section">
        <div className="related-content-card">
          ...
          <div className="related-grid">
            ...
            <Link href="/calculators/electric/kw-to-volts" className="related-card">
              <h3 className="related-title">{tRelated('kwToVolts')}</h3>
              <p className="related-desc">{tRelated('kwToVoltsDesc')}</p>
            </Link>
          </div>
        </div>
      </div>
