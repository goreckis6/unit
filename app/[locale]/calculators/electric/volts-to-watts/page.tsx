...
import { VoltsToWattsCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.voltsToWatts?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.voltsToWatts.seo' });
  const baseUrl = 'https://unitconverterhub.com';
  const path = locale === 'en' ? '/calculators/electric/volts-to-watts' : `/${locale}/calculators/electric/volts-to-watts`;
  const canonicalUrl = `${baseUrl}${path}`;
  const hreflangUrls = generateHreflangUrls('/calculators/electric/volts-to-watts');
  return {
    title: t('title'),
...
export default async function VoltsToWattsPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.voltsToWatts' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });
  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.voltsToWatts.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.voltsToWatts.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.voltsToWatts.seo.related' });
