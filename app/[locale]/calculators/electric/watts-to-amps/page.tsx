...
import { WattsToAmpsCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.wattsToAmps?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.wattsToAmps.seo' });
  const baseUrl = 'https://unitconverterhub.com';
  const path = locale === 'en' ? '/calculators/electric/watts-to-amps' : `/${locale}/calculators/electric/watts-to-amps`;
  const canonicalUrl = `${baseUrl}${path}`;
  const hreflangUrls = generateHreflangUrls('/calculators/electric/watts-to-amps');
  return {
    title: t('title'),
...
export default async function WattsToAmpsPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.wattsToAmps' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });
  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.wattsToAmps.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.wattsToAmps.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.wattsToAmps.seo.related' });
