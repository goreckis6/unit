...
import { WattsToKwhCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.wattsToKwh?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.wattsToKwh.seo' });
  const baseUrl = 'https://unitconverterhub.com';
  const path = locale === 'en' ? '/calculators/electric/watts-to-kwh' : `/${locale}/calculators/electric/watts-to-kwh`;
  const canonicalUrl = `${baseUrl}${path}`;
  const hreflangUrls = generateHreflangUrls('/calculators/electric/watts-to-kwh');
...
export default async function WattsToKwhPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.wattsToKwh' });
  ...
  const tSeo = await getTranslations({ locale, namespace: 'calculators.wattsToKwh.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.wattsToKwh.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.wattsToKwh.seo.related' });
      ...
      <div className="related-calculators-section">
        <div className="related-content-card">
          ...
          <div className="related-grid">
            ...
            <Link href="/calculators/electric/watts-to-joules" className="related-card">
              <h3 className="related-title">{tRelated('wattsToJoules')}</h3>
              <p className="related-desc">{tRelated('wattsToJoulesDesc')}</p>
            </Link>
