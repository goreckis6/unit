...
import { WattsToVaCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.wattsToVa?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.wattsToVa.seo' });
  const baseUrl = 'https://unitconverterhub.com';
  const path = locale === 'en' ? '/calculators/electric/watts-to-va' : `/${locale}/calculators/electric/watts-to-va`;
  const canonicalUrl = `${baseUrl}${path}`;
  const hreflangUrls = generateHreflangUrls('/calculators/electric/watts-to-va');
...
export default async function WattsToVaPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.wattsToVa' });
  ...
  const tSeo = await getTranslations({ locale, namespace: 'calculators.wattsToVa.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.wattsToVa.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.wattsToVa.seo.related' });
      ...
      <div className="related-calculators-section">
        <div className="related-content-card">
          ...
          <div className="related-grid">
            ...
            <Link href="/calculators/electric/kw-to-va" className="related-card">
              <h3 className="related-title">{tRelated('kwToVa')}</h3>
              <p className="related-desc">{tRelated('kwToVaDesc')}</p>
            </Link>
