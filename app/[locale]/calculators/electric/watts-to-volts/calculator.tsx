...
export function wattsToVolts(
  watts: number,
  amps: number,
  powerFactor: number,
  type: PhaseType
): number {
  if (watts < 0 || amps <= 0) return 0;
  if (type === 'dc') return watts / amps;
  if (powerFactor <= 0 || powerFactor > 1) return 0;
  if (type === 'three') return watts / (Math.sqrt(3) * amps * powerFactor);
  return watts / (amps * powerFactor);
}

export function WattsToVoltsCalculator() {
  const t = useTranslations('calculators.wattsToVolts');
  ...
  const handleCalculate = () => {
    ...
    if (!isNaN(w) && !isNaN(a) && w >= 0 && a > 0) {
      if (type === 'dc') {
        setResult(wattsToVolts(w, a, 1, 'dc'));
      } else if (!isNaN(pf) && pf > 0 && pf <= 1) {
        setResult(wattsToVolts(w, a, pf, type));
      } else {
        setResult(null);
      }
    } else {
