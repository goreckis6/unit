...
import { foodCalculators } from '@/lib/calculators/food';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.foodCalculators?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const tSeo = await getTranslations({ locale, namespace: 'calculators.foodCalculators.seoMeta' });
  const path = '/calculators/food';
...
export default async function FoodCalculatorsPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.foodCalculators.faq' });
  const tSeo = await getTranslations({ locale, namespace: 'calculators.foodCalculators.seoContent' });
  
  const faqItems = await getFaqItems(locale);
  const calculators = await getCalculatorsForCategory('food', locale, foodCalculators);
        ...
        <div className="container">
          <div className="calculator-header">
            ...
            <div className="header-content">
              <div className="title-badge">{t('foodCalculators.badge') || 'Food'}</div>
              <h1 className="page-title">{t('foodCalculators.title')}</h1>
            </div>
          </div>
