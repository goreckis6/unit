...
import { ConvolutionCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.convolution?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.convolution.seo' });
  const path = '/calculators/math/convolution';
...
export default async function ConvolutionPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.convolution' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.convolution.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.convolution.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.convolution.seo.related' });
