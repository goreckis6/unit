...
import { CosineCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.cosine?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.cosine.seo' });
  const path = '/calculators/math/cosine';
...
export default async function CosinePage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.cosine' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.cosine.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.cosine.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.cosine.seo.related' });
