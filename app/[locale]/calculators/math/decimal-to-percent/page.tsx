...
import { DecimalToPercentCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.decimalToPercent?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.decimalToPercent.seo' });
  const path = '/calculators/math/decimal-to-percent';
...
export default async function DecimalToPercentPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.decimalToPercent' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.decimalToPercent.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.decimalToPercent.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.decimalToPercent.seo.related' });
        ...
        <div className="container">
          <div className="faq-content-card">
            ...
            <div className="related-grid">
              ...
              <Link href="/calculators/math/percent-error" className="related-card">
