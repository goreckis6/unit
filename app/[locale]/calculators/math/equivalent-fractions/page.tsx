...
import { EquivalentFractionsCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.equivalentFractions?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.equivalentFractions.seo' });
  const path = '/calculators/math/equivalent-fractions';
...
export default async function EquivalentFractionsPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.equivalentFractions' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.equivalentFractions.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.equivalentFractions.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.equivalentFractions.seo.related' });
        ...
        <div className="container">
          <div className="faq-content-card">
            ...
            <div className="related-grid">
              ...
              <Link href="/calculators/math/complex-fractions" className="related-card">
                <h3 className="related-title">{tRelated('complexFractions')}</h3>
                <p className="related-desc">{tRelated('complexFractionsDesc')}</p>
              </Link>
            </div>
          </div>
        </div>
