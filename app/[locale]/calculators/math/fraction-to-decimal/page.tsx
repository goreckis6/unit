...
import { FractionToDecimalCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.fractionToDecimal?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.fractionToDecimal.seo' });
  const path = '/calculators/math/fraction-to-decimal';
...
export default async function FractionToDecimalPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.fractionToDecimal' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.fractionToDecimal.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.fractionToDecimal.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.fractionToDecimal.seo.related' });
