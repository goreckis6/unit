...
export function FractionToPercentCalculator() {
  const t = useTranslations('calculators.fractionToPercent');
  ...
  const handleCalculate = () => {
    ...
    setCalc({ ok: true, raw, simplified, decimal, percent, decimals: places });
  };
  ...
  const formatted = useMemo(() => {
    ...
    const percentString = toFixedTrimmed(calc.percent, calc.decimals);
    const simplifiedText = formatFraction(calc.simplified);
    return { decimalString, percentString, simplifiedText };
  }, [calc]);
              ...
              >
                <div className="result-item">
                  ...
                  <div className="number-input result-value-box">
                    <span className="result-value">{formatted.percentString}%</span>
                    <CopyButton text={`${formatted.percentString}%`} />
                  </div>
                </div>
                ...
                <div className="seo-content-card" style={{ marginTop: '0.25rem' }}>
                  ...
                  <div className="example-text" style={{ lineHeight: 1.8 }}>
                    ...
                    <p>
                      <strong>{t('stepPercent')}</strong>
                    </p>
                    <p>{formatted.decimalString} × 100 = {formatted.percentString}%</p>
                  </div>
                </div>
              </div>
            )}
