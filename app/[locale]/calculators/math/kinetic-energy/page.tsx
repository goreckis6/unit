...
import { KineticEnergyCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.kineticEnergy?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.kineticEnergy.seo' });
  const path = '/calculators/math/kinetic-energy';
...
export default async function KineticEnergyPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.kineticEnergy' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.kineticEnergy.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.kineticEnergy.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.kineticEnergy.seo.related' });
      ...
      <div className="related-calculators-section">
        <div className="related-content-card">
          ...
          <div className="related-grid">
            <Link href="/calculators/math/pythagorean-theorem" className="related-card">
              <h3 className="related-title">{tRelated('pythagorean')}</h3>
              <p className="related-desc">{tRelated('pythagoreanDesc')}</p>
            </Link>
