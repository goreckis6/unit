...
import { LongDivisionCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.longDivision?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.longDivision.seo' });
  const path = '/calculators/math/long-division';
...
export default async function LongDivisionPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.longDivision' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.longDivision.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.longDivision.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.longDivision.seo.related' });
          ...
          <div className="seo-content-card">
            ...
            <div className="seo-paragraphs">
              ...
              <div className="seo-example">
                <h3 className="example-heading">{tSeo('partsHeading')}</h3>
                <ul style={{ margin: 0, paddingLeft: '1.25rem', color: 'var(--text-secondary)', display: 'grid', gap: '0.4rem' }}>
                  <li>{tSeo('partsItem1')}</li>
                  <li>{tSeo('partsItem2')}</li>
                  <li>{tSeo('partsItem3')}</li>
                  <li>{tSeo('partsItem4')}</li>
                </ul>
              </div>
              <div className="seo-example">
