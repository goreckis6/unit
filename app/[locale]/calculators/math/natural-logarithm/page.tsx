...
import { NaturalLogarithmCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.naturalLogarithm?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.naturalLogarithm.seo' });
  const path = '/calculators/math/natural-logarithm';
...
export default async function NaturalLogarithmPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.naturalLogarithm' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.naturalLogarithm.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.naturalLogarithm.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.naturalLogarithm.seo.related' });
