...
import { PercentErrorCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.percentError?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.percentError.seo' });
  const path = '/calculators/math/percent-error';
...
export default async function PercentErrorPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.percentError' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.percentError.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.percentError.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.percentError.seo.related' });
