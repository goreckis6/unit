...
import { PercentageIncreaseCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.percentageIncrease?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.percentageIncrease.seo' });
  const path = '/calculators/math/percentage-increase';
...
export default async function PercentageIncreasePage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.percentageIncrease' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.percentageIncrease.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.percentageIncrease.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.percentageIncrease.seo.related' });
      ...
      <div className="related-calculators-section">
        <div className="related-content-card">
          ...
          <div className="related-grid">
            ...
            <Link href="/calculators/math/ratio" className="related-card">
              <h3 className="related-title">{tRelated('ratio')}</h3>
              <p className="related-desc">{tRelated('ratioDesc')}</p>
            </Link>
