...
import { QuadraticEquationCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.quadraticEquation?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.quadraticEquation.seo' });
  const path = '/calculators/math/quadratic-equation';
...
export default async function QuadraticEquationPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.quadraticEquation' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });
  const tSeo = await getTranslations({ locale, namespace: 'calculators.quadraticEquation.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.quadraticEquation.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.quadraticEquation.seo.related' });
