...
import { SubtractingFractionsCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.subtractingFractions?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.subtractingFractions.seo' });
  const path = '/calculators/math/subtracting-fractions';
...
export default async function SubtractingFractionsPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.subtractingFractions' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.subtractingFractions.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.subtractingFractions.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.subtractingFractions.seo.related' });
      ...
      <div className="calculator-container">
        <div className="container">
          <div className="calculator-card">
            <SubtractingFractionsCalculator />
          </div>
        </div>
      </div>
