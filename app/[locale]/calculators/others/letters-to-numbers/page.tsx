...
import { LettersToNumbersConverter } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.lettersToNumbers?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.lettersToNumbers.seo' });
  const path = '/calculators/others/letters-to-numbers';
...
export default async function LettersToNumbersPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.lettersToNumbers' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.lettersToNumbers.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.lettersToNumbers.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.lettersToNumbers.seo.related' });
