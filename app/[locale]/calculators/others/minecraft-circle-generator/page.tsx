...
import { MinecraftCircleGeneratorCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.minecraftCircleGenerator?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.minecraftCircleGenerator.seo' });
  const path = '/calculators/others/minecraft-circle-generator';
...
export default async function MinecraftCircleGeneratorPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.minecraftCircleGenerator' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.minecraftCircleGenerator.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.minecraftCircleGenerator.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.minecraftCircleGenerator.seo.related' });
  ...
  return (
    <>
      ...
      <div className="calculator-container">
        <div className="container">
          <div className="calculator-card">
            <MinecraftCircleGeneratorCalculator />
          </div>
        </div>
      </div>
      ...
      {/* Related Calculators – styled like FAQ, above FAQ */}
