...
import { MirrorTextGenerator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.mirrorText?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.mirrorText.seo' });
  const path = '/calculators/others/mirror-text';
...
export default async function MirrorTextPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.mirrorText' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.mirrorText.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.mirrorText.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.mirrorText.seo.related' });
