...
import { MulchCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.mulchCalculator?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.mulchCalculator.seo' });
  const path = '/calculators/others/mulch-calculator';
...
export default async function MulchCalculatorPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.mulchCalculator' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.mulchCalculator.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.mulchCalculator.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.mulchCalculator.seo.related' });
      ...
      <div className="calculator-header">
        <div className="container">
          ...
          <div className="header-content">
            <div className="title-badge">{t('categoryBadge')}</div>
        ...
        <div className="container">
          <div className="faq-content-card">
            ...
            <div className="related-grid">
              <Link href="/calculators/others/minecraft-circle-generator" className="related-card">
                <h3 className="related-title">{tRelated('minecraftCircle')}</h3>
                <p className="related-desc">{tRelated('minecraftCircleDesc')}</p>
              </Link>
