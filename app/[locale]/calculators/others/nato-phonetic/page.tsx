...
import { NatoPhoneticCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.natoPhonetic?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.natoPhonetic.seo' });
  const path = '/calculators/others/nato-phonetic';
...
export default async function NatoPhoneticPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.natoPhonetic' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.natoPhonetic.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.natoPhonetic.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.natoPhonetic.seo.related' });
        ...
        <div className="related-content-card">
          ...
          <div className="related-grid">
            ...
            <Link href="/calculators/others/mirror-text" className="related-card">
              ...
              <p className="related-desc">{tRelated('mirrorTextDesc')}</p>
            </Link>
          </div>
        </div>
      </div>
