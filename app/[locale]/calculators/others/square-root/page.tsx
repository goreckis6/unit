...
import { SquareRootCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.squareRoot?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.squareRoot.seo' });
  const path = '/calculators/others/square-root';
...
export default async function SquareRootPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.squareRoot' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.squareRoot.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.squareRoot.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.squareRoot.seo.related' });
      ...
      <div className="calculator-header">
        <div className="container">
          ...
          <div className="header-content">
            <div className="title-badge">Others</div>
