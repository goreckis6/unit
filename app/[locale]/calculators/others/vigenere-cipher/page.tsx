...
import { VigenereCipherCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.vigenereCipher?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.vigenereCipher.seo' });
  const path = '/calculators/others/vigenere-cipher';
...
export default async function VigenereCipherPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.vigenereCipher' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.vigenereCipher.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.vigenereCipher.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.vigenereCipher.seo.related' });
      ...
      <div className="calculator-header">
        <div className="container">
          ...
          <div className="header-content">
            <div className="title-badge">Text Tool</div>
