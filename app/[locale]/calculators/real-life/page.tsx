...
import { realLifeCalculators } from '@/lib/calculators/real-life';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.realLifeCalculators?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const tSeo = await getTranslations({ locale, namespace: 'calculators.realLifeCalculators.seoMeta' });
  const path = '/calculators/real-life';
...
export default async function RealLifeCalculatorsPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.realLifeCalculators.faq' });
  const tSeo = await getTranslations({ locale, namespace: 'calculators.realLifeCalculators.seoContent' });

  const faqItems = await getFaqItems(locale);
  const calculators = await getCalculatorsForCategory('real-life', locale, realLifeCalculators);
        ...
        <div className="container">
          <div className="calculator-header">
            ...
            <div className="header-content">
              <div className="title-badge">{t('realLifeCalculators.badge') || 'Real-life'}</div>
              <h1 className="page-title">{t('realLifeCalculators.title')}</h1>
            </div>
          </div>
