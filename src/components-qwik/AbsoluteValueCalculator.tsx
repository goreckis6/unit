...
export default component$(() => {
  ...
  const calculate = $(() => {
    ...
    if (isNaN(num)) {
      error.value = t('calculators.absoluteValue.error.invalidInput');
      return;
    }
    
    try {
      result.value = Math.abs(num);
    } catch (e) {
      error.value = t('calculators.absoluteValue.error.calculationError');
    }
  });
  ...
  const calculatorName = t('calculators.absoluteValue.title');
  const calculatorDescription = t('calculators.absoluteValue.seo.description');
      ...
      <div class="container">
        <div class="calculator-header">
          ...
          <div class="header-content">
            <div class="title-badge">{t('calculators.absoluteValue.badge')}</div>
            <h1 class="page-title">{t('calculators.absoluteValue.title')}</h1>
            <p class="page-description">{t('calculators.absoluteValue.description')}</p>
          </div>
        </div>
        ...
        <div class="calculator-container">
          <div class="calculator-card">
            <div class="input-section">
              <div class="input-card">
                <label class="input-label">{t('calculators.absoluteValue.inputLabel')}</label>
                <div class="input-wrapper">
                  <div class="input-prefix">|</div>
                  <input
                    type="number"
                    class="number-input"
                    placeholder={t('calculators.absoluteValue.placeholder')}
              ...
              <div class="action-buttons">
                <button onClick$={calculate} class="btn btn-primary" disabled={!canCalculate.value}>
                  ...
                  <span>{t('calculators.absoluteValue.calculate')}</span>
                </button>
                <button onClick$={clear} class="btn btn-secondary">
                  <svg class="btn-icon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M18 6L6 18M6 6L18 18" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                  </svg>
                  <span>{t('calculators.absoluteValue.clear')}</span>
                </button>
              </div>
            </div>
              ...
              <div class="result-section">
                <div class="result-header">
                  <div class="result-badge">
                    ...
                    {t('calculators.absoluteValue.result')}
                  </div>
                </div>
          ...
          <div class="seo-content-section">
            <div class="seo-content-card">
              <h2 class="seo-heading">{t('calculators.absoluteValue.seo.content.heading')}</h2>
              
              <div class="seo-paragraphs">
                <p class="seo-paragraph">
                  {t('calculators.absoluteValue.seo.content.paragraph1')}
                </p>
                
                <p class="seo-paragraph">
                  {t('calculators.absoluteValue.seo.content.paragraph2')}
                </p>
                
                <p class="seo-paragraph">
                  {t('calculators.absoluteValue.seo.content.paragraph3')}
                </p>
                
                <div class="seo-example">
                  <h3 class="example-heading">{t('calculators.absoluteValue.seo.content.exampleHeading')}</h3>
                  <p class="example-text">{t('calculators.absoluteValue.seo.content.exampleText')}</p>
                </div>
              </div>
            </div>
          </div>
...
export const head: DocumentHead = ({ resolveValue }) => {
  ...
  return {
    title: t('calculators.absoluteValue.seo.title'),
    meta: [
      {
        name: 'description',
        content: t('calculators.absoluteValue.seo.description'),
      },
      {
        name: 'keywords',
        content: t('calculators.absoluteValue.seo.keywords'),
      },
    ],
  };
};
