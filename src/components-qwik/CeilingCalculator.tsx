...
export default component$(() => {
  ...
  const calculate = $(() => {
    ...
    if (isNaN(num)) {
      error.value = t('calculators.ceiling.error.invalidInput');
      return;
    }
    
    try {
      result.value = Math.ceil(num);
    } catch (e) {
      error.value = t('calculators.ceiling.error.calculationError');
    }
  });
  ...
  const calculatorName = t('calculators.ceiling.title');
  const calculatorDescription = t('calculators.ceiling.seo.description');
      ...
      <div class="container">
        <div class="calculator-header">
          ...
          <div class="header-content">
            <div class="title-badge">{t('calculators.ceiling.badge')}</div>
            <h1 class="page-title">{t('calculators.ceiling.title')}</h1>
            <p class="page-description">{t('calculators.ceiling.description')}</p>
          </div>
        </div>
        ...
        <div class="calculator-container">
          <div class="calculator-card">
            <div class="input-section">
              <div class="input-card">
                <label class="input-label">{t('calculators.ceiling.inputLabel')}</label>
                <div class="input-wrapper">
                  <div class="input-prefix">⌈</div>
                  <input
                    type="number"
                    class="number-input"
                    placeholder={t('calculators.ceiling.placeholder')}
              ...
              <div class="action-buttons">
                <button onClick$={calculate} class="btn btn-primary" disabled={!canCalculate.value}>
                  ...
                  <span>{t('calculators.ceiling.calculate')}</span>
                </button>
                <button onClick$={clear} class="btn btn-secondary">
                  <svg class="btn-icon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M18 6L6 18M6 6L18 18" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                  </svg>
                  <span>{t('calculators.ceiling.clear')}</span>
                </button>
              </div>
            </div>
          ...
          <div class="seo-content-section">
            <div class="seo-content-card">
              <h2 class="seo-heading">{t('calculators.ceiling.seo.content.heading')}</h2>
              
              <div class="seo-paragraphs">
                <p class="seo-paragraph">
                  {t('calculators.ceiling.seo.content.paragraph1')}
                </p>
                
                <p class="seo-paragraph">
                  {t('calculators.ceiling.seo.content.paragraph2')}
                </p>
                
                <p class="seo-paragraph">
                  {t('calculators.ceiling.seo.content.paragraph3')}
                </p>
                
                <div class="seo-example">
                  <h3 class="example-heading">{t('calculators.ceiling.seo.content.exampleHeading')}</h3>
                  <p class="example-text">{t('calculators.ceiling.seo.content.exampleText')}</p>
                </div>
              </div>
            </div>
          </div>
...
export const head: DocumentHead = ({ resolveValue }) => {
  ...
  return {
    title: t('calculators.ceiling.seo.title'),
    meta: [
      {
        name: 'description',
        content: t('calculators.ceiling.seo.description'),
      },
      {
        name: 'keywords',
        content: t('calculators.ceiling.seo.keywords'),
      },
    ],
  };
};
