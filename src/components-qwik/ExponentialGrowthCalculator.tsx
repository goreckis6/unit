...
export default component$(() => {
  const initialValue = useSignal<number | null>(null);
  const growthRate = useSignal<number | null>(null);
  ...
  useTask$(({ track }) => {
    track(() => initialValue.value);
    track(() => growthRate.value);
    track(() => time.value);
    canCalculate.value =
      initialValue.value !== null && initialValue.value !== '' &&
      growthRate.value !== null && growthRate.value !== '' &&
      time.value !== null && time.value !== '';
  });
  ...
  const calculate = $(() => {
    ...
    if (!canCalculate.value) {
      error.value = t('calculators.exponentialGrowth.error.invalidInput');
      return;
    }
    
    const initial = parseFloat(String(initialValue.value)) || 0;
    const rate = parseFloat(String(growthRate.value)) || 0;
    const t = parseFloat(String(time.value)) || 0;
    
    if (isNaN(initial) || isNaN(rate) || isNaN(t)) {
      error.value = t('calculators.exponentialGrowth.error.invalidInput');
      return;
    }
    ...
    try {
      ...
      const calculated = initial * Math.exp(rate * t);
      
      if (!isFinite(calculated)) {
        error.value = t('calculators.exponentialGrowth.error.resultTooLarge');
        return;
      }
      
      result.value = calculated;
    } catch (e) {
      error.value = t('calculators.exponentialGrowth.error.calculationError');
    }
  });
  ...
  const calculatorName = t('calculators.exponential-growth.title');
  const calculatorDescription = t('calculators.exponential-growth.seo.description');
            ...
            <div class="input-section">
              <div class="input-card">
                ...
                <input
                  ...
                  onInput$={(e) => {
                    ...
                    initialValue.value = target.value ? parseFloat(target.value) : null;
                  }}
                  onKeyUp$={(e) => {
                    if (e.key === 'Enter') {
                      calculate();
              ...
              <div class="input-card">
                ...
                <input
                  ...
                  onInput$={(e) => {
                    const target = e.target as HTMLInputElement;
                    growthRate.value = target.value ? parseFloat(target.value) : null;
                  }}
                  onKeyUp$={(e) => {
                    if (e.key === 'Enter') {
                      calculate();
              ...
              <div class="input-card">
                <label class="input-label">{t('calculators.exponentialGrowth.time')}</label>
                <input
                  type="number"
                  class="number-input"
                  placeholder={t('calculators.exponentialGrowth.enterTime')}
                  value={time.value ?? ''}
                  onInput$={(e) => {
                    const target = e.target as HTMLInputElement;
                    time.value = target.value ? parseFloat(target.value) : null;
                  }}
                  onKeyUp$={(e) => {
                    if (e.key === 'Enter') {
                      calculate();
...
export const head: DocumentHead = ({ resolveValue }) => {
  ...
  return {
    title: t('calculators.exponentialGrowth.seo.title'),
    meta: [
      {
        name: 'description',
        content: t('calculators.exponentialGrowth.seo.description'),
      },
      {
        name: 'keywords',
        content: t('calculators.exponentialGrowth.seo.keywords'),
      },
    ],
  };
};
