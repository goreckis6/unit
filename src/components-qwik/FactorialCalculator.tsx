...
import { getFaqSchema } from '../utils/faqSchema';
...
export default component$(() => {
  ...
  const { t, getTranslation } = useTranslate();
  
  const canCalculate = useSignal(false);
  
  useTask$(({ track }) => {
    track(() => number.value);
    canCalculate.value = number.value !== null && number.value !== '' && number.value >= 0 && number.value <= 170;
  });
  ...
  const calculate = $(() => {
    ...
    if (isNaN(num)) {
      error.value = t('calculators.factorial.error.invalidInput');
      return;
    }
    
    if (num < 0) {
      error.value = t('calculators.factorial.error.negativeNumber');
      result.value = null;
      return;
    }
    
    if (num > 170) {
      error.value = t('calculators.factorial.error.numberTooLarge');
      result.value = null;
      return;
    }
    
    if (num !== Math.floor(num)) {
      error.value = t('calculators.factorial.error.nonInteger');
      result.value = null;
      return;
    }
    
    try {
      const calculated = factorial(num);
      ...
      error.value = t('calculators.factorial.error.calculationError');
    }
  });
  ...
  // Get FAQ items from translations and generate schema
  const faqItemsRaw = getTranslation('calculators.factorial.seo.faq.items');
  ...
  const calculatorName = t('calculators.factorial.title');
  const calculatorDescription = t('calculators.factorial.seo.description');
      ...
      <div class="container">
        <div class="calculator-header">
          ...
          <div class="header-content">
            <div class="title-badge">{t('calculators.factorial.badge')}</div>
            <h1 class="page-title">{t('calculators.factorial.title')}</h1>
            <p class="page-description">{t('calculators.factorial.description')}</p>
          </div>
        </div>
        ...
        <div class="calculator-container">
              ...
              <div class="square-row">
                <div class="number-card">
                  <div class="number-card-header">
                    <div class="number-badge">n</div>
                    <label class="number-label">{t('calculators.factorial.number')}</label>
                  </div>
                  <input
                    type="number"
                    class="number-input-large"
                    placeholder={t('calculators.factorial.enterNumber')}
                    value={number.value ?? ''}
                    onInput$={(e) => {
              ...
              <div class="result-section">
                <div class="result-header">
                  <div class="result-badge">
                    ...
                    {t('calculators.factorial.result')}
                  </div>
                </div>
          ...
          <div class="seo-content-section">
            <div class="seo-content-card">
              <h2 class="seo-heading">{t('calculators.factorial.seo.content.heading')}</h2>
              
              <div class="seo-paragraphs">
                <p class="seo-paragraph">
                  {t('calculators.factorial.seo.content.paragraph1')}
                </p>
                
                <p class="seo-paragraph">
                  {t('calculators.factorial.seo.content.paragraph2')}
                </p>
                
                <p class="seo-paragraph">
                  {t('calculators.factorial.seo.content.paragraph3')}
                </p>
                
                <div class="seo-example">
                  <h3 class="example-heading">{t('calculators.factorial.seo.content.exampleHeading')}</h3>
                  <p class="example-text">{t('calculators.factorial.seo.content.exampleText')}</p>
                </div>
              </div>
            </div>
          </div>
...
export const head: DocumentHead = ({ resolveValue }) => {
  ...
  return {
    title: t('calculators.factorial.seo.title'),
    meta: [
      {
        name: 'description',
        content: t('calculators.factorial.seo.description'),
      },
      {
        name: 'keywords',
        content: t('calculators.factorial.seo.keywords'),
      },
    ],
  };
};
