...
export default component$(() => {
  ...
  const calculate = $(() => {
    ...
    if (isNaN(num)) {
      error.value = t('calculators.floor.error.invalidInput');
      return;
    }
    
    try {
      result.value = Math.floor(num);
    } catch (e) {
      error.value = t('calculators.floor.error.calculationError');
    }
  });
  ...
  const calculatorName = t('calculators.floor.title');
  const calculatorDescription = t('calculators.floor.seo.description');
      ...
      <div class="container">
        <div class="calculator-header">
          ...
          <div class="header-content">
            <div class="title-badge">{t('calculators.floor.badge')}</div>
            <h1 class="page-title">{t('calculators.floor.title')}</h1>
            <p class="page-description">{t('calculators.floor.description')}</p>
          </div>
        </div>
        ...
        <div class="calculator-container">
          <div class="calculator-card">
            <div class="input-section">
              <div class="input-card">
                <label class="input-label">{t('calculators.floor.inputLabel')}</label>
                <div class="input-wrapper">
                  <div class="input-prefix">⌊</div>
                  <input
                    type="number"
                    class="number-input"
                    placeholder={t('calculators.floor.placeholder')}
              ...
              <div class="action-buttons">
                <button onClick$={calculate} class="btn btn-primary" disabled={!canCalculate.value}>
                  ...
                  <span>{t('calculators.floor.calculate')}</span>
                </button>
                <button onClick$={clear} class="btn btn-secondary">
                  <svg class="btn-icon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M18 6L6 18M6 6L18 18" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                  </svg>
                  <span>{t('calculators.floor.clear')}</span>
                </button>
              </div>
            </div>
          ...
          <div class="seo-content-section">
            <div class="seo-content-card">
              <h2 class="seo-heading">{t('calculators.floor.seo.content.heading')}</h2>
              
              <div class="seo-paragraphs">
                <p class="seo-paragraph">
                  {t('calculators.floor.seo.content.paragraph1')}
                </p>
                
                <p class="seo-paragraph">
                  {t('calculators.floor.seo.content.paragraph2')}
                </p>
                
                <p class="seo-paragraph">
                  {t('calculators.floor.seo.content.paragraph3')}
                </p>
                
                <div class="seo-example">
                  <h3 class="example-heading">{t('calculators.floor.seo.content.exampleHeading')}</h3>
                  <p class="example-text">{t('calculators.floor.seo.content.exampleText')}</p>
                </div>
              </div>
            </div>
          </div>
...
export const head: DocumentHead = ({ resolveValue }) => {
  ...
  return {
    title: t('calculators.floor.seo.title'),
    meta: [
      {
        name: 'description',
        content: t('calculators.floor.seo.description'),
      },
      {
        name: 'keywords',
        content: t('calculators.floor.seo.keywords'),
      },
    ],
  };
};
