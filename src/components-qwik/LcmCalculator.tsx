...
export default component$(() => {
  ...
  const calculate = $(() => {
    ...
    if (isNaN(a) || isNaN(b)) {
      error.value = t('calculators.lcm.error.invalidInput');
      return;
    }
    
    if (a === 0 || b === 0) {
      error.value = t('calculators.lcm.error.zeroNotAllowed');
      result.value = null;
      return;
    }
    ...
    try {
      ...
      const calculated = (Math.abs(a) * Math.abs(b)) / gcd(a, b);
      
      if (!isFinite(calculated)) {
        error.value = t('calculators.lcm.error.invalidResult');
        return;
      }
      
      result.value = calculated;
    } catch (e) {
      error.value = t('calculators.lcm.error.calculationError');
    }
  });
  ...
  const calculatorName = t('calculators.lcm.title');
  const calculatorDescription = t('calculators.lcm.seo.description');
              ...
              <div class="addition-row">
                <div class="number-card">
                  <div class="number-card-header">
                    ...
                    <label class="number-label">{t('calculators.lcm.firstNumber')}</label>
                  </div>
                  <input
                    type="number"
                    class="number-input-large"
                    placeholder={t('calculators.lcm.enterFirstNumber')}
                ...
                <div class="number-card">
                  <div class="number-card-header">
                    ...
                    <label class="number-label">{t('calculators.lcm.secondNumber')}</label>
                  </div>
                  <input
                    type="number"
                    class="number-input-large"
                    placeholder={t('calculators.lcm.enterSecondNumber')}
                ...
                <div class="result-display">
                  <div class="result-formula-large">
                    <div class="formula-number-box">
                      <span class="formula-number-value">LCM({formatNumber(firstNumber.value)}, {formatNumber(secondNumber.value)})</span>
                    </div>
...
export const head: DocumentHead = ({ resolveValue }) => {
  ...
  return {
    title: t('calculators.lcm.seo.title'),
    meta: [
      {
        name: 'description',
        content: t('calculators.lcm.seo.description'),
      },
      {
        name: 'keywords',
        content: t('calculators.lcm.seo.keywords'),
      },
    ],
  };
};
