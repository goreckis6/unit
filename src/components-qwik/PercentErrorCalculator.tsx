...
export default component$(() => {
  const actualValue = useSignal<number | null>(null);
  const theoreticalValue = useSignal<number | null>(null);
  ...
  useTask$(({ track }) => {
    track(() => actualValue.value);
    track(() => theoreticalValue.value);
    canCalculate.value =
      actualValue.value !== null && actualValue.value !== '' &&
      theoreticalValue.value !== null && theoreticalValue.value !== '' &&
      theoreticalValue.value !== 0;
  });
  ...
  const calculate = $(() => {
    ...
    if (!canCalculate.value) {
      if (theoreticalValue.value === 0) {
        error.value = t('calculators.percentError.error.theoreticalCannotBeZero');
      } else {
        error.value = t('calculators.percentError.error.invalidInput');
      }
      return;
    }
    
    const actual = parseFloat(String(actualValue.value));
    const theoretical = parseFloat(String(theoreticalValue.value));
    ...
    result.value = Math.abs((actual - theoretical) / theoretical) * 100;
  });
  
  const clear = $(() => {
    actualValue.value = null;
    theoreticalValue.value = null;
    result.value = null;
    error.value = '';
  });
  ...
  const calculatorName = t('calculators.percent-error.title');
  const calculatorDescription = t('calculators.percent-error.seo.description');
              ...
              <div class="percentage-row">
                <div class="number-card">
                  <div class="number-card-header">
                    ...
                    <label class="number-label">{t('calculators.percentError.actualValue')}</label>
                  </div>
                  <input
                    type="number"
                    class="number-input-large"
                    placeholder={t('calculators.percentError.enterActualValue')}
                    value={actualValue.value ?? ''}
                    onInput$={(e) => {
                      const target = e.target as HTMLInputElement;
                      actualValue.value = target.value ? parseFloat(target.value) : null;
                    }}
                    onKeyUp$={(e) => {
                      if (e.key === 'Enter') {
                        calculate();
                ...
                <div class="operator-card">
                  <div class="operator-symbol">vs</div>
                </div>

                <div class="number-card">
                  <div class="number-card-header">
                    <div class="number-badge">T</div>
                    <label class="number-label">{t('calculators.percentError.theoreticalValue')}</label>
                  </div>
                  <input
                    type="number"
                    class="number-input-large"
                    placeholder={t('calculators.percentError.enterTheoreticalValue')}
                    value={theoreticalValue.value ?? ''}
                    onInput$={(e) => {
                      const target = e.target as HTMLInputElement;
                      theoreticalValue.value = target.value ? parseFloat(target.value) : null;
                    }}
                    onKeyUp$={(e) => {
                      if (e.key === 'Enter') {
                        calculate();
                ...
                <div class="result-display">
                  <div class="result-formula-large">
                    <div class="formula-number-box">
                      <span class="formula-number-value">|{formatNumber(actualValue.value)} - {formatNumber(theoreticalValue.value)}|</span>
                    </div>
                    <div class="formula-operator-box">
                      <span class="formula-operator-symbol">/</span>
                    </div>
                    <div class="formula-number-box">
                      <span class="formula-number-value">{formatNumber(theoreticalValue.value)}</span>
                    </div>
                    <div class="formula-equals-box">
                      <span class="formula-equals-symbol">× 100%</span>
                    </div>
                    <div class="formula-equals-box">
...
export const head: DocumentHead = ({ resolveValue }) => {
  ...
  return {
    title: t('calculators.percentError.seo.title'),
    meta: [
      {
        name: 'description',
        content: t('calculators.percentError.seo.description'),
      },
      {
        name: 'keywords',
        content: t('calculators.percentError.seo.keywords'),
      },
    ],
  };
};
