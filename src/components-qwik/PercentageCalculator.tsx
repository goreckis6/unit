...
export default component$(() => {
  const percentage = useSignal<number | null>(null);
  ...
  const calculate = $(() => {
    ...
    const percent = parseFloat(String(percentage.value)) || 0;
    const num = parseFloat(String(number.value)) || 0;
    
    if (isNaN(percent) || isNaN(num)) {
      error.value = t('calculators.percentage.error.invalidInput');
      return;
    }
    
    try {
      const calculated = (percent / 100) * num;
      
      if (!isFinite(calculated)) {
        error.value = t('calculators.percentage.error.invalidResult');
        return;
      }
      
      result.value = calculated;
    } catch (e) {
      error.value = t('calculators.percentage.error.calculationError');
    }
  });
  ...
  const calculatorName = t('calculators.percentage.title');
  const calculatorDescription = t('calculators.percentage.seo.description');
      ...
      <div class="container">
        <div class="calculator-header">
          ...
          <div class="header-content">
            <div class="title-badge">{t('calculators.percentage.badge')}</div>
            <h1 class="page-title">{t('calculators.percentage.title')}</h1>
            <p class="page-description">{t('calculators.percentage.description')}</p>
          </div>
        </div>
        ...
        <div class="calculator-container">
              ...
              <div class="percentage-row">
                <div class="number-card">
                  <div class="number-card-header">
                    ...
                    <label class="number-label">{t('calculators.percentage.percentage')}</label>
                  </div>
                  <input
                    type="number"
                    class="number-input-large"
                    placeholder={t('calculators.percentage.enterPercentage')}
                    value={percentage.value ?? ''}
                    onInput$={(e) => {
                      const target = e.target as HTMLInputElement;
                      percentage.value = target.value ? parseFloat(target.value) : null;
                      calculate();
                    }}
                    onKeyUp$={(e) => {
                      if (e.key === 'Enter') {
                ...
                <div class="operator-card">
                  <div class="operator-symbol">{t('calculators.percentage.of')}</div>
                </div>
                
                <div class="number-card">
                  <div class="number-card-header">
                    <div class="number-badge">#</div>
                    <label class="number-label">{t('calculators.percentage.number')}</label>
                  </div>
                  <input
                    type="number"
                    class="number-input-large"
                    placeholder={t('calculators.percentage.enterNumber')}
                    value={number.value ?? ''}
                    onInput$={(e) => {
                      const target = e.target as HTMLInputElement;
                ...
                <div class="result-display">
                  <div class="result-formula-large">
                    <div class="formula-number-box">
                      <span class="formula-number-value">{formatNumber(percentage.value)}%</span>
                    </div>
                    <div class="formula-operator-box">
                      <span class="formula-operator-symbol">{t('calculators.percentage.of')}</span>
                    </div>
                    <div class="formula-number-box">
          ...
          <div class="seo-content-section">
            <div class="seo-content-card">
              <h2 class="seo-heading">{t('calculators.percentage.info.title')}</h2>
              
              <div class="seo-paragraphs">
                <p class="seo-paragraph">
                  {t('calculators.percentage.info.description')}
                </p>
                
                <div class="seo-example">
                  <h3 class="example-heading">{t('calculators.percentage.info.example')}</h3>
                  <p class="example-text">{t('calculators.percentage.info.exampleText')}</p>
                </div>
              </div>
            </div>
          </div>
...
export const head: DocumentHead = ({ resolveValue }) => {
  ...
  return {
    title: t('calculators.percentage.seo.title'),
    meta: [
      {
        name: 'description',
        content: t('calculators.percentage.seo.description'),
      },
      {
        name: 'keywords',
        content: t('calculators.percentage.seo.keywords'),
      },
    ],
  };
};
