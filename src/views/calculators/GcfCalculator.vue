          ...
          <div class="input-section">
            <div class="input-row">
              <div class="input-card">
                <label class="input-label">{{ $t('calculators.gcf.firstNumber') }}</label>
                <input
                  v-model.number="number1"
                  type="number"
                  class="number-input"
                  :placeholder="$t('calculators.gcf.placeholder')"
                  @input="validateInput"
              ...
              <div class="input-card">
                <label class="input-label">{{ $t('calculators.gcf.secondNumber') }}</label>
                <input
            ...
            <div class="action-buttons">
              <button @click="calculate" class="btn btn-primary" :disabled="!canCalculate">
                ...
                <span>{{ $t('calculators.gcf.calculate') }}</span>
              </button>
              <button @click="clear" class="btn btn-secondary">
                <svg class="btn-icon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M18 6L6 18M6 6L18 18" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
                <span>{{ $t('calculators.gcf.clear') }}</span>
              </button>
            </div>
          </div>
          ...
          <transition name="slide-up">
            <div v-if="result !== null" class="result-section">
              ...
              <div class="result-display">
                <div class="gcf-result">
                  <div class="result-formula">
                    <span class="formula-text">GCF({{ number1 }}, {{ number2 }})</span>
              ...
              <div v-if="result !== null" class="info-section">
                <div class="info-card">
                  <div class="info-item">
                    <span class="info-label">{{ $t('calculators.gcf.firstNumber') }}:</span>
                    <span class="info-value">{{ number1 }}</span>
                  </div>
                  <div class="info-item">
                    <span class="info-label">{{ $t('calculators.gcf.secondNumber') }}:</span>
                    <span class="info-value">{{ number2 }}</span>
                  </div>
                  <div class="info-item">
                    <span class="info-label">{{ $t('calculators.gcf.gcf') }}:</span>
              ...
              <div v-if="steps.length > 0" class="steps-section">
                <div class="steps-header">
                  ...
                  <h4 class="steps-title">{{ $t('calculators.gcf.stepByStep') }}</h4>
                </div>
        ...
        <div class="seo-content-section">
          <div class="seo-content-card">
            ...
            <div class="seo-paragraphs">
              ...
              <p class="seo-paragraph">
                {{ $t('calculators.gcf.seo.content.paragraph4') }}
              </p>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    updateMetaTags() {
      ...
      document.title = this.$t('calculators.gcf.seo.title')
      ...
      metaDescription.setAttribute('content', this.$t('calculators.gcf.seo.description'))
      ...
      metaKeywords.setAttribute('content', this.$t('calculators.gcf.seo.keywords'))
    ...
    updateOpenGraph() {
      ...
      ogTitle.setAttribute('content', this.$t('calculators.gcf.seo.title'))
      ...
      ogDescription.setAttribute('content', this.$t('calculators.gcf.seo.description'))
    ...
    validateInput() {
      ...
      if ((this.number1 !== null && this.number1 <= 0) || 
          (this.number2 !== null && this.number2 <= 0)) {
        this.error = this.$t('calculators.gcf.error.positiveOnly')
      }
    },
    calculateGCF(a, b) {
      a = Math.abs(a)
      ...
      while (b !== 0) {
        const remainder = a % b
        steps.push(`${this.$t('calculators.gcf.steps.euclideanStep')} ${a} ÷ ${b} = ${Math.floor(a / b)} remainder ${remainder}`)
        a = b
        b = remainder
      }
      
      return { result: a, steps, originalA, originalB }
    },
    calculate() {
      this.error = ''
      this.steps = []
      ...
      if (this.number1 === null || this.number1 <= 0 || 
          this.number2 === null || this.number2 <= 0) {
        this.error = this.$t('calculators.gcf.error.invalidInput')
        return
      }
      ...
      const calculation = this.calculateGCF(a, b)
      this.result = calculation.result
      
      // Add steps
      this.steps.push(...calculation.steps)
      
      // Final step
      this.steps.push(
        `${this.$t('calculators.gcf.steps.result')} GCF(${a}, ${b}) = ${this.result}`
      )
    },
    clear() {
      this.number1 = null
