              ...
              <div class="result-display">
                ...
                <div class="result-formula-large">
                  <div class="formula-number-box">
                    <span class="formula-number-value">{{ formatNumber(oldValue) }}</span>
                  </div>
                  <div class="formula-operator-box">
                    <span class="formula-operator-symbol">→</span>
                  </div>
                  <div class="formula-number-box">
                    <span class="formula-number-value">{{ formatNumber(newValue) }}</span>
                  </div>
                  <div class="formula-equals-box">
                    <span class="formula-equals-symbol">=</span>
                  </div>
                  <div class="result-value-box-large" :class="{ 'positive': result > 0, 'negative': result < 0 }">
                    <span class="result-value-large">{{ formatResult(result) }}%</span>
                  </div>
                </div>
                
                <!-- Change Type -->
                <div class="change-type" v-if="result !== null">
                  <div class="change-badge" :class="{ 'increase': result > 0, 'decrease': result < 0, 'no-change': result === 0 }">
                    <span v-if="result > 0">{{ $t('calculators.percentageChange.increase') }}</span>
                    <span v-else-if="result < 0">{{ $t('calculators.percentageChange.decrease') }}</span>
                    <span v-else>{{ $t('calculators.percentageChange.noChange') }}</span>
                  </div>
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    calculate() {
      ...
      const old = parseFloat(this.oldValue) || 0
      const newVal = parseFloat(this.newValue) || 0
      ...
      this.result = ((newVal - old) / old) * 100
    },
    clear() {
      this.oldValue = null
      this.newValue = null
    ...
    updateMetaTags() {
      ...
      document.title = this.$t('calculators.percentageChange.seo.title')
      ...
      metaDescription.setAttribute('content', this.$t('calculators.percentageChange.seo.description'))
      ...
      metaKeywords.setAttribute('content', this.$t('calculators.percentageChange.seo.keywords'))
...
.result-value-box-large.positive {
  background: linear-gradient(135deg, rgba(34, 197, 94, 0.9) 0%, rgba(22, 163, 74, 0.9) 100%);
  box-shadow: 0 10px 32px rgba(34, 197, 94, 0.4);
}

.result-value-box-large.negative {
