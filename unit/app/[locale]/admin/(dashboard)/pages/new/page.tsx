...
function parseCalculatorLabels(val: unknown): Record<string, string> {
  ...
  if (typeof val === 'string') {
    try {
      ...
      return parseCalculatorLabels(parsed);
    } catch {
      return {};
    }
  }
...
export default function AdminNewPage() {
  ...
  async function handleGeneratePost(provider: 'ollama' | 'claude' = 'ollama') {
    ...
    try {
      ...
      const msg = err instanceof Error
        ? (err.name === 'AbortError' ? 'Timeout — limit 48 h. Spróbuj ponownie.' : err.message)
        : 'Błąd generowania';
      setError(msg);
    } finally {
  ...
  async function handleTranslateToOtherLocales(forceAll = false, onlyOne = false) {
    ...
    try {
      for (let i = 0; i < localesToTranslate.length; i++) {
        ...
        updateTranslation(loc, 'content', data.content ?? '');
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const res = await fetch(
          `/api/twojastara/pages/check-slug?category=${encodeURIComponent(cat)}&slug=${encodeURIComponent(sl)}`
        );
        const data = await res.json();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch('/api/twojastara/pages', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '1rem', color: 'var(--text-primary)' }}>New Page</h1>
            ...
            {(ADMIN_LOCALES ?? []).map((l) => (
              <button
                ...
                style={{
                  ...
                  background: activeLocale === l ? '#6366f1' : '#1e293b',
                  color: activeLocale === l ? 'white' : '#94a3b8',
                  cursor: 'pointer',
                  fontSize: '0.75rem',
                }}
              >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.25rem', flexWrap: 'wrap' }}>
                ...
                {activeLocale === 'en' && (
                  <>
                    <span style={{ fontSize: '0.7rem', color: 'var(--text-secondary)' }} title="Ollama: 48 h. Claude: ~2 min.">
                      Generate:
                    </span>
                    <button
                      type="button"
                      ...
                      <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginTop: '0.25rem' }}>
                        {activeLocale !== 'en' && (
                          <button
                            ...
                            className="btn btn-secondary btn-sm"
                            title="Translate labels from EN to this language (Ollama). Limit: 48 h. Retry: 2x."
                          >
          ...
          {showCalculatorSection && (
            <div style={{ paddingTop: '1rem', borderTop: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <p style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', margin: 0 }}>
                Link to an existing calculator or paste custom calculator code. Code will appear on a separate subpage: /pages/[slug]/calculator
              </p>
              <div>
                <label className="admin-form-label">Link to existing calculator</label>
                <select
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                  <label className="admin-form-label" style={{ marginBottom: 0 }}>Calculator code (TSX/JS) — shown on /calculator subpage</label>
                  <button
                    type="button"
                ...
                <textarea
                  ...
                  placeholder="Paste or edit calculator code (example pre-filled)"
                  rows={12}
                  className="admin-form-textarea"
