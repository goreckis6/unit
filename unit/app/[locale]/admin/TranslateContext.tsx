...
function shouldAutoResumeTranslateError(message: string): boolean {
  ...
  if (s.includes('usage limit') || s.includes('quota exceeded')) return false;
  return (
...
export function TranslateProvider({ children }: { children: ReactNode }) {
  ...
  const startTranslate = useCallback(async (params: {
    ...
    const processPage = async (page: Page): Promise<boolean> => {
      ...
      // Normal+ and Fast mode both batch 2 locales for shorter content.
      // Long Ollama content stays single-locale so chunked translation remains stable.
      const LOCALE_BATCH_SIZE = (translateProvider === 'ollama' && enContent.length > 1_400) ? 1 : 2;
