...
import { C1V1Calculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.c1v1?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.c1v1.seo' });
  const path = '/calculators/biology/c1v1-calculator';
...
export default async function C1V1Page({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.c1v1' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.c1v1.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.c1v1.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.c1v1.seo.related' });
