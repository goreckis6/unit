...
import { conversionCalculators } from '@/lib/calculators/conversion';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.conversionCalculators?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const tSeo = await getTranslations({ locale, namespace: 'calculators.conversionCalculators.seoMeta' });
  const path = '/calculators/conversion';
...
export default async function ConversionCalculatorsPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.conversionCalculators.faq' });
  const tSeo = await getTranslations({ locale, namespace: 'calculators.conversionCalculators.seoContent' });
  
  const faqItems = await getFaqItems(locale);
  const calculators = await getCalculatorsForCategory('conversion', locale, conversionCalculators);
        ...
        <div className="container">
          <div className="calculator-header">
            ...
            <div className="header-content">
              <div className="title-badge">{t('conversionCalculators.badge') || 'Conversion'}</div>
              <h1 className="page-title">{t('conversionCalculators.title')}</h1>
            </div>
          </div>
