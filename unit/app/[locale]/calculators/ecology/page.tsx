...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.ecologyCalculators?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const tSeo = await getTranslations({ locale, namespace: 'calculators.ecologyCalculators.seoMeta' });
  const path = '/calculators/ecology';
...
export default async function EcologyCalculatorsPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.ecologyCalculators.faq' });
  ...
  const [calculators, session] = await Promise.all([
    getCalculatorsForCategory('ecology', locale, ecologyCalculators),
    getSession(),
  ]);
        ...
        <div className="container">
          <div className="calculator-header">
            ...
            <div className="header-content">
              <div className="title-badge">{t('ecologyCalculators.badge') || 'Ecology'}</div>
              <h1 className="page-title">{t('ecologyCalculators.title')}</h1>
            </div>
          </div>
