...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.financeCalculators?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const tSeo = await getTranslations({ locale, namespace: 'calculators.financeCalculators.seoMeta' });
  const path = '/calculators/finance';
...
export default async function FinanceCalculatorsPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.financeCalculators.faq' });
  ...
  const [calculators, session] = await Promise.all([
    getCalculatorsForCategory('finance', locale, financeCalculators),
    getSession(),
  ]);
        ...
        <div className="container">
          <div className="calculator-header">
            ...
            <div className="header-content">
              <div className="title-badge">{t('financeCalculators.badge') || 'Finance'}</div>
              <h1 className="page-title">{t('financeCalculators.title')}</h1>
            </div>
          </div>
