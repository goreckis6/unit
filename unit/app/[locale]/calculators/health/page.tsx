...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.healthCalculators?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const tSeo = await getTranslations({ locale, namespace: 'calculators.healthCalculators.seoMeta' });
  const path = '/calculators/health';
...
export default async function HealthCalculatorsPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.healthCalculators.faq' });
  ...
  const [calculators, session] = await Promise.all([
    getCalculatorsForCategory('health', locale, healthCalculators),
    getSession(),
  ]);
        ...
        <div className="container">
          <div className="calculator-header">
            ...
            <div className="header-content">
              <div className="title-badge">{t('healthCalculators.badge') || 'Health'}</div>
              <h1 className="page-title">{t('healthCalculators.title')}</h1>
            </div>
          </div>
