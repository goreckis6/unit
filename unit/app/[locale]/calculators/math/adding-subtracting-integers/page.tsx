...
import { AddingSubtractingIntegersCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.addingSubtractingIntegers?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.addingSubtractingIntegers.seo' });
  const path = '/calculators/math/adding-subtracting-integers';
...
export default async function AddingSubtractingIntegersPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.addingSubtractingIntegers' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.addingSubtractingIntegers.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.addingSubtractingIntegers.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.addingSubtractingIntegers.seo.related' });
      ...
      <div className="calculator-container">
        <div className="container">
          <div className="calculator-card">
            <AddingSubtractingIntegersCalculator />
          </div>
        </div>
      </div>
