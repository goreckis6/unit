...
import { GCFCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.gcf?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.gcf.seo' });
  const path = '/calculators/math/gcf';
...
export default async function GCFPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.gcf' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.gcf.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.gcf.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.gcf.seo.related' });
      ...
      <div className="related-calculators-section">
        <div className="related-content-card">
          ...
          <div className="related-grid">
            <Link href="/calculators/math/lcm" className="related-card">
              <h3 className="related-title">{tRelated('lcm')}</h3>
              <p className="related-desc">{tRelated('lcmDesc')}</p>
            </Link>
