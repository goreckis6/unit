...
import { PolynomialRemainderCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.polynomialRemainder?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.polynomialRemainder.seo' });
  const path = '/calculators/math/polynomial-remainder';
...
export default async function PolynomialRemainderPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.polynomialRemainder' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.polynomialRemainder.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.polynomialRemainder.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.polynomialRemainder.seo.related' });
        ...
        <div className="related-content-card">
          ...
          <div className="related-grid">
            <Link href="/calculators/math/roots" className="related-card">
              <h3 className="related-title">{tRelated('roots')}</h3>
              <p className="related-desc">{tRelated('rootsDesc')}</p>
            </Link>
            ...
            <Link href="/calculators/math/convolution" className="related-card">
              <h3 className="related-title">{tRelated('convolution')}</h3>
              <p className="related-desc">{tRelated('convolutionDesc')}</p>
            </Link>
          </div>
        </div>
      </div>
