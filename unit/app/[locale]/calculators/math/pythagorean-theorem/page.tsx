...
import { PythagoreanTheoremCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.pythagoreanTheorem?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.pythagoreanTheorem.seo' });
  const path = '/calculators/math/pythagorean-theorem';
...
export default async function PythagoreanTheoremPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.pythagoreanTheorem' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });
  const tSeo = await getTranslations({ locale, namespace: 'calculators.pythagoreanTheorem.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.pythagoreanTheorem.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.pythagoreanTheorem.seo.related' });
        ...
        <div className="container">
          <div className="faq-content-card">
            ...
            <div className="related-grid">
              ...
              <Link href="/calculators/math/quadratic-equation" className="related-card">
                <h3 className="related-title">{tRelated('quadraticEquation')}</h3>
                <p className="related-desc">{tRelated('quadraticEquationDesc')}</p>
              </Link>
            </div>
          </div>
        </div>
