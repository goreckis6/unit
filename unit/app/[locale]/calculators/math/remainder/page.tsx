...
import { RemainderCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.remainder?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.remainder.seo' });
  const path = '/calculators/math/remainder';
...
export default async function RemainderPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.remainder' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.remainder.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.remainder.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.remainder.seo.related' });
      ...
      <div className="related-calculators-section">
        <div className="related-content-card">
          ...
          <div className="related-grid">
            <Link href="/calculators/math/polynomial-remainder" className="related-card">
              <h3 className="related-title">{tRelated('polynomialRemainder')}</h3>
              <p className="related-desc">{tRelated('polynomialRemainderDesc')}</p>
            </Link>
