...
import { MorseCodeCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.morseCode?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.morseCode.seo' });
  const path = '/calculators/others/morse-code';
...
export default async function MorseCodePage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.morseCode' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.morseCode.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.morseCode.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.morseCode.seo.related' });
      ...
      <div className="calculator-header">
        <div className="container">
          ...
          <div className="header-content">
            <div className="title-badge">Other Tools</div>
