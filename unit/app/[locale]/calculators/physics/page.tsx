...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.physicsCalculators?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const tSeo = await getTranslations({ locale, namespace: 'calculators.physicsCalculators.seoMeta' });
  const path = '/calculators/physics';
...
export default async function PhysicsCalculatorsPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.physicsCalculators.faq' });
  ...
  const [calculators, session] = await Promise.all([
    getCalculatorsForCategory('physics', locale, physicsCalculators),
    getSession(),
  ]);
        ...
        <div className="container">
          <div className="calculator-header">
            ...
            <div className="header-content">
              <div className="title-badge">{t('physicsCalculators.badge') || 'Physics'}</div>
              <h1 className="page-title">{t('physicsCalculators.title')}</h1>
            </div>
          </div>
