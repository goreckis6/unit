...
import { AgeInDaysCalculator } from './calculator';
...
async function getFaqItems(locale: string): Promise<FaqItem[]> {
  try {
    ...
    return (messages.default?.calculators?.ageInDays?.seo?.faq?.items as FaqItem[]) || [];
  } catch {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.ageInDays.seo' });
  const path = '/calculators/real-life/age-in-days';
...
export default async function AgeInDaysPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'calculators.ageInDays' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });

  const faqItems = await getFaqItems(locale);
  const tSeo = await getTranslations({ locale, namespace: 'calculators.ageInDays.seo.content' });
  const tFaq = await getTranslations({ locale, namespace: 'calculators.ageInDays.seo.faq' });
  const tRelated = await getTranslations({ locale, namespace: 'calculators.ageInDays.seo.related' });
      ...
      <div className="related-calculators-section">
        <div className="related-content-card">
          ...
          <div className="related-grid">
            <Link href="/calculators/real-life/reverse-text-generator" className="related-card">
              <h3 className="related-title">{tRelated('reverseTextGenerator')}</h3>
              <p className="related-desc">{tRelated('reverseTextGeneratorDesc')}</p>
            </Link>
